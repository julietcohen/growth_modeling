...
parms <- list(r = r, K = K, g = g) # naomi did this in her code (https://naomitague.github.io/ESM232_course/lectures/lecture10_moredynamic.html#(7)), I am not sure how we should change this variable for our purposes

# **How does C know how to build on itself?**
# run the ODE solver
C_over_time = ode(y = 10, #iniital condition, then the next time stamp, ODE uses the initial condition and the change in C
            times = simtimes, 
            func = C_growth,
            parms = parms)
...
ggplot(as.data.frame(C_over_time), aes(Year, Carbon)) + 
  ...
  theme_light() +
  ggtitle("Forest Carbon over time") +
   xlab("Year") + 
   ylab("Carbon") +
...
#### Why are we doing a second set of samples?
#### How is this testing the maximum and mean C values?
...
c_wrapper = function(r, g, K, C, simtimes, func) {
    parms = list(r = r, C = C, g = g, K = K) 
...
ggplot(tmp, aes(metric, value, col = metric)) +
  geom_boxplot() +
  theme_light() +
  ggtitle("Metrics for Forest Growth: maximum and mean growth") +
   xlab("Metric") + 
   ylab("Carbon") +
   theme(axis.title.x = element_text(color = "black", size = 11),
         axis.text.x = element_text(face = "bold", color = "black", size = 10),
...
```
```{r}
rownames(sens_C_maxpop$S) = c("r","g", "K")
sens_C_maxpop$S
```
## Graph the main effect without covariance 

```{r}
data <- data.frame(S = sens_C_maxpop$S$original, T = sens_C_maxpop$T$original, parameter = c("r", "g", "K")) %>% 
  pivot_longer(cols = c(S, T))

class(sens_C_maxpop$S)
colnames(sens_C_maxpop$S)
rownames(sens_C_maxpop$S)
sens_C_maxpop$S$original
variables <- rownames(sens_C_maxpop$S)

ggplot(data, aes(x = name, y = value, color = parameter)) +
  geom_point() +
  theme_light() +
  ggtitle("Metrics for Forest Growth: maximum and mean growth") +
   xlab("Metric") + 
...
```{r}
rownames(sens_C_maxpop$T) = c("r","g", "K")
sens_C_maxpop$T
```
...
#I think the order of the results is r, g, K? not totally sure if this is accurate
```
...
Assuming that X1 = r, when looking at the first order variance, both the mean population and the max population are highly correlated with the r (X1) variable. Again when we look at the total sensitivty this trend holds, the r (x1) the early exponential growth rate, has the highest correlation with the max and mean populations.
